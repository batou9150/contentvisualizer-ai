  ...
  return (
    <div className="bg-white dark:bg-[#1e293b] p-1 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800 overflow-hidden">
      <div className="flex bg-gray-50 dark:bg-[#0f172a] border-b border-gray-100 dark:border-gray-800 p-1">
        ...
        <button onClick={() => setInputMode('image')} className={`flex-1 py-2 px-4 rounded-xl text-sm font-semibold transition-all ${inputMode === 'image' ? 'bg-white dark:bg-[#1e293b] shadow-sm text-indigo-600 dark:text-indigo-400' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200'}`}>From Image</button>
      </div>
      <div className="p-6">
        <h2 className="text-lg font-semibold mb-4 dark:text-white">
          {inputMode === 'url' ? 'Paste Article URL' : inputMode === 'text' ? 'Paste Article Content' : 'Upload Content Image'}
        </h2>
        <form onSubmit={onSubmit} className="flex flex-col gap-4">
          {inputMode === 'url' ? (
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end gap-3">
                <button disabled={loading || !imageData} type="submit" className="px-8 py-3 bg-indigo-600 text-white font-semibold rounded-xl hover:bg-indigo-700 disabled:opacity-50 transition-all flex items-center gap-2 shadow-[0_4px_20px_rgba(79,70,229,0.5)]">
                  {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><LightningIcon className="w-5 h-5" />Analyze Image</>}
                </button>
              </div>
            </div>
          )}
