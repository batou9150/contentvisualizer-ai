    ...
    <section className="bg-white dark:bg-[#1e293b] p-4 sm:p-8 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-0 mb-6">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
          <button onClick={() => setShowRawCode(!showRawCode)} className="flex-1 sm:flex-none justify-center text-xs px-3 py-2 sm:py-1.5 rounded-lg border border-gray-200 dark:border-gray-800 hover:border-indigo-300 dark:hover:border-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-all font-medium text-gray-600 dark:text-gray-400 flex items-center gap-1.5">
            {showRawCode ? <>Hide Code</> : <>View Code</>}
          </button>
          <button
            onClick={onGenerateImage}
            disabled={isGeneratingSummaryImage}
            className="flex-1 sm:flex-none justify-center px-4 py-2 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-600 hover:text-white dark:hover:bg-indigo-600 dark:hover:text-white text-xs font-bold rounded-lg transition-all flex items-center gap-2 disabled:opacity-50 shadow-sm"
          >
      ...
      <VisualResult url={summaryImageUrl} driveUrl={summaryDriveUrl} onImprove={onImproveImage} isImproving={isImprovingImage} />
